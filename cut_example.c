...
,[C]),green_cut(1,2,D),format(green_cut(1,2,B) = ~w
,[D]) */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__0 = create_var(state->next_var_id++);
    ...
    if (!green_cut_3(state, create_int(1), create_int(2), var__3)) { state->failed = true; break; }
    if (!format_2(state, create_atom("green_cut(1,2,B) = ~w\n"), create_list(var__3, create_nil()))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
