...
bool safe_member_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    ...
    /* Clause 2: safe_member(A,[A|B]) :- ! */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__0 = create_var(state->next_var_id++);
        term_t* var__1 = create_var(state->next_var_id++);
        if (unify(state, var__0, arg1) &&
            unify(state, create_list(var__0, var__1), arg2)) {
    ...
    /* Clause 3: safe_member(A,[B|C]) :- safe_member(A,C) */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__0 = create_var(state->next_var_id++);
        term_t* var__1 = create_var(state->next_var_id++);
        ...
        if (unify(state, var__0, arg1) &&
            unify(state, create_list(var__1, var__2), arg2)) {

            do {
    if (!safe_member_2(state, var__0, var__2)) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool not_member_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    ...
    /* Clause 2: not_member(A,[B|C]) :- A\=B,not_member(A,C) */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__0 = create_var(state->next_var_id++);
        term_t* var__1 = create_var(state->next_var_id++);
        ...
        if (unify(state, var__0, arg1) &&
            ...
            do {
    if (!neq_2(state, var__0, var__1)) { state->failed = true; break; }
    if (!not_member_2(state, var__0, var__2)) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool green_cut_3(prolog_state_t* state, term_t* arg1, term_t* arg2, term_t* arg3) {
    /* Clause 1: green_cut(A,B,C) :- A>=B,!,C=A */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__0 = create_var(state->next_var_id++);
        term_t* var__1 = create_var(state->next_var_id++);
        ...
        if (unify(state, var__0, arg1) &&
            unify(state, var__1, arg2) &&
            unify(state, var__2, arg3)) {

            do {
    if (!gte_2(state, var__0, var__1)) { state->failed = true; break; }
    perform_cut(state);
    if (!eq_2(state, var__2, var__0)) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
),max(5,3,A),format(max(5, 3) = ~w
,[A]),max(2,7,B),format(max(2, 7) = ~w
,[B]),safe_member(C,[2,3,4]),format(safe_member(A,[2,3,4]) = ~w
,[C]),not_member(1,[2,3]),format(not_member(1,[2,3])
,[]),green_cut(3,2,D),format(green_cut(3,2,B) = ~w
,[D]),safe_member([c,E],[[a,a],[b,b],[c,c]]),format(member([c,C],[[a,a],[b,b],[c,c]]) = ~w
,[E]) */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__0 = create_var(state->next_var_id++);
        term_t* var__1 = create_var(state->next_var_id++);
        term_t* var__2 = create_var(state->next_var_id++);
        term_t* var__3 = create_var(state->next_var_id++);
        term_t* var__4 = create_var(state->next_var_id++);
        if (true) {

            do {
    if (!write_1(state, create_atom("Testing cut behavior\n"))) { state->failed = true; break; }
    if (!max_3(state, create_int(5), create_int(3), var__0)) { state->failed = true; break; }
    if (!format_2(state, create_atom("max(5, 3) = ~w\n"), create_list(var__0, create_nil()))) { state->failed = true; break; }
    if (!max_3(state, create_int(2), create_int(7), var__1)) { state->failed = true; break; }
    if (!format_2(state, create_atom("max(2, 7) = ~w\n"), create_list(var__1, create_nil()))) { state->failed = true; break; }
    if (!safe_member_2(state, var__2, create_list(create_int(2), create_list(create_int(3), create_list(create_int(4), create_nil()))))) { state->failed = true; break; }
    if (!format_2(state, create_atom("safe_member(A,[2,3,4]) = ~w\n"), create_list(var__2, create_nil()))) { state->failed = true; break; }
    if (!not_member_2(state, create_int(1), create_list(create_int(2), create_list(create_int(3), create_nil())))) { state->failed = true; break; }
    if (!format_2(state, create_atom("not_member(1,[2,3])\n"), create_nil())) { state->failed = true; break; }
    if (!green_cut_3(state, create_int(3), create_int(2), var__3)) { state->failed = true; break; }
    if (!format_2(state, create_atom("green_cut(3,2,B) = ~w\n"), create_list(var__3, create_nil()))) { state->failed = true; break; }
    if (!safe_member_2(state, create_list(create_atom("c"), create_list(var__4, create_nil())), create_list(create_list(create_atom("a"), create_list(create_atom("a"), create_nil())), create_list(create_list(create_atom("b"), create_list(create_atom("b"), create_nil())), create_list(create_list(create_atom("c"), create_list(create_atom("c"), create_nil())), create_nil()))))) { state->failed = true; break; }
    if (!format_2(state, create_atom("member([c,C],[[a,a],[b,b],[c,c]]) = ~w\n"), create_list(var__4, create_nil()))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
