...
bool nth_3(prolog_state_t* state, term_t* arg1, term_t* arg2, term_t* arg3) {
    ...
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__604 = create_var(state->next_var_id++);
        term_t* var__606 = create_var(state->next_var_id++);
        term_t* var__610 = create_var(state->next_var_id++);
        term_t* var__614 = create_var(state->next_var_id++);
        term_t* var__630 = create_var(state->next_var_id++);
        if (unify(state, var__610, arg1) &&
            unify(state, create_list(var__604, var__606), arg2) &&
            unify(state, var__614, arg3)) {

            do {
    if (!gt_2(state, var__610, create_int(0))) { state->failed = true; break; }
    if (!is_2(state, var__630, create_compound("-", 2, (term_t*[]){var__610, create_int(1)}))) { state->failed = true; break; }
    if (!nth_3(state, var__630, var__606, var__614)) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool positive_numbers_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    ...
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__670 = create_var(state->next_var_id++);
        term_t* var__672 = create_var(state->next_var_id++);
        term_t* var__680 = create_var(state->next_var_id++);
        if (unify(state, create_list(var__670, var__672), arg1) &&
            unify(state, create_list(var__670, var__680), arg2)) {

            do {
    if (!gt_2(state, var__670, create_int(0))) { state->failed = true; break; }
    perform_cut(state);
    if (!positive_numbers_2(state, var__672, var__680)) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
    ...
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__722 = create_var(state->next_var_id++);
        term_t* var__724 = create_var(state->next_var_id++);
        term_t* var__730 = create_var(state->next_var_id++);
        if (unify(state, create_list(var__722, var__724), arg1) &&
            unify(state, var__730, arg2)) {

            do {
    if (!positive_numbers_2(state, var__724, var__730)) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
