...
bool reachable_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    /* Clause 1: reachable(_2774,_2774) */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__2774 = create_var(state->next_var_id++);
        if (unify(state, var__2774, arg1) &&
    ...
    /* Clause 2: reachable(_2788,_2790) :- path(_2788,_2796),reachable(_2796,_2790) */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__2788 = create_var(state->next_var_id++);
        term_t* var__2790 = create_var(state->next_var_id++);
        term_t* var__2796 = create_var(state->next_var_id++);
        if (unify(state, var__2788, arg1) &&
            unify(state, var__2790, arg2)) {

            do {
    if (!path_2(state, var__2788, var__2796)) { state->failed = true; break; }
    if (!reachable_2(state, var__2796, var__2790)) { state->failed = true; break; }
            } while (0);
...
bool all_colors_1(prolog_state_t* state, term_t* arg1) {
    /* Clause 1: all_colors(_2826) :- findall(_2830,color(_2830),_2826) */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__2826 = create_var(state->next_var_id++);
        term_t* var__2830 = create_var(state->next_var_id++);
        if (unify(state, var__2826, arg1)) {

            do {
    /* findall(_2830, color(_2830), _2826) */
    {
        ...
        /* Enumerate solutions */
        while (true) {
    if (!color_1(state, var__2830)) { state->failed = true; break; }
...
bool first_solution_1(prolog_state_t* state, term_t* arg1) {
    /* Clause 1: first_solution(_2856) :- color(_2856),! */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__2856 = create_var(state->next_var_id++);
        if (unify(state, var__2856, arg1)) {

            do {
    if (!color_1(state, var__2856)) { state->failed = true; break; }
    perform_cut(state);
            } while (0);
            if (!state->failed) return true;
        }
...
),findall(_2888,color(_2888),_2896),format(All colors: ~w
,[_2896]),reachable(a,d),write(Path from a to d exists
),first_solution(_2888),format(First color: ~w
,[_2888]) */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__2888 = create_var(state->next_var_id++);
        term_t* var__2896 = create_var(state->next_var_id++);
        if (true) {

            do {
    if (!write_1(state, create_atom("Testing nondeterministic predicates\n"))) { state->failed = true; break; }
    /* findall(_2888, color(_2888), _2896) */
    {
        term_t** solutions = NULL;
        int solution_count = 0;
        prolog_state_t findall_state;
        ...
        /* Enumerate solutions */
        while (true) {
    if (!color_1(state, var__2888)) { state->failed = true; break; }
    ...
    if (!format_2(state, create_atom("All colors: ~w\n"), create_list(var__2896, create_nil()))) { state->failed = true; break; }
    ...
    if (!first_solution_1(state, var__2888)) { state->failed = true; break; }
    if (!format_2(state, create_atom("First color: ~w\n"), create_list(var__2888, create_nil()))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
