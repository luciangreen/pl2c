...
/* Sentinel clause index indicating all clauses for a predicate have been exhausted */
#define CLAUSE_EXHAUSTED_SENTINEL 9999
...
typedef struct {
    ...
    bool failed;
    int next_var_id;
    bool backtracking;  /* True when backtracking to find the next solution */
} prolog_state_t;
...
%% translate_nondeterministic_predicate(+Clauses, +FuncName, +Params, -CCode)
% Generates code for a predicate with multiple clauses that supports backtracking via choice points
translate_nondeterministic_predicate(Clauses, FuncName, Params, CCode) :-
    length(Clauses, NumClauses),
...
'bool ~w(prolog_state_t* state~w) {
    ...
    int start_clause = 1;
    if (state->backtracking && state->choice_stack &&
        state->choice_stack->predicate_id == (int)(intptr_t)&~w) {
        ...
        if (start_clause >= CLAUSE_EXHAUSTED_SENTINEL) {
            state->failed = true;
            return false;
        }
    }
...
translate_single_clause_with_choicepoint((Head :- Body), FuncName, Index, IsLast, CCode) :-
    ...
    setup_call_cleanup(
        ...
        (
            ...
            NextIndex is Index + 1,
            ( IsLast ->
                % Last clause: push sentinel (CLAUSE_EXHAUSTED_SENTINEL) to signal exhaustion on backtrack
                format(atom(CCode),
'    /* Clause ~w: ~w :- ~w */
    if (start_clause <= ~w) {
        int saved_bindings_size = state->bindings.size;
        /* Push sentinel choice point before trying last clause */
        push_choice_point(state, (int)(intptr_t)&~w, CLAUSE_EXHAUSTED_SENTINEL);
~w~w
            do {
~w            } while (0);
            if (!state->failed) return true;
        }
        /* Clause failed: remove the sentinel choice point */
        if (state->choice_stack &&
            state->choice_stack->predicate_id == (int)(intptr_t)&~w) {
            pop_choice_point(state);
        }
...
', [Index, HeadDisp, BodyDisp, Index, FuncName, VarDecls, HeadCode, BodyCode, FuncName])
            ;
                % Not last clause: push choice point for next clause
                format(atom(CCode),
'    /* Clause ~w: ~w :- ~w */
    if (start_clause <= ~w) {
        int saved_bindings_size = state->bindings.size;
...
~w            } while (0);
        ...
        /* Clause failed: remove choice point for this clause */
        if (state->choice_stack &&
            state->choice_stack->predicate_id == (int)(intptr_t)&~w) {
            pop_choice_point(state);
        }
...
translate_body(findall(Template, Goal, Result), CCode, Depth) :-
    !,
    % findall/3: enumerate all solutions using isolated findall_state
    % Generate goal code (uses 'state' references which we replace with 'findall_state')
    translate_body(Goal, GoalCodeTemplate, Depth),
    % Replace state references in GoalCode to use findall_state:
    %   'state->' -> 'findall_state.'  (for field access)
    %   '(state,' -> '(&findall_state,'  (for function calls with args)
    %   '(state)' -> '(&findall_state)'  (for no-arg function calls)
    atom_codes(GoalCodeTemplate, GoalCodes),
    atom_codes('state->', StateArrowCodes),
    atom_codes('findall_state.', FindallStateDotCodes),
    replace_all_occurrences(GoalCodes, StateArrowCodes, FindallStateDotCodes, TempCodes1),
...
'    /* findall/3 */
    {
        ...
        /* Save initial bindings for restoration between iterations */
        int initial_bindings_size = findall_state.bindings.size;
        int initial_bindings_capacity = findall_state.bindings.capacity;
        binding_t* initial_bindings_copy = NULL;
        if (initial_bindings_size > 0) {
            initial_bindings_copy = malloc(sizeof(binding_t) * initial_bindings_capacity);
            if (initial_bindings_copy) {
                memcpy(initial_bindings_copy, findall_state.bindings.bindings,
                       sizeof(binding_t) * initial_bindings_size);
            }
        }
        ...
        while (true) {
            ...
            /* Execute the goal in findall_state */
~w
            ...
            if (solutions) {
                int var_offset = findall_state.next_var_id;
            ...
            /* Restore bindings to before the goal was called */
            if (findall_state.bindings.bindings) free(findall_state.bindings.bindings);
            findall_state.bindings.size = initial_bindings_size;
            findall_state.bindings.capacity = initial_bindings_capacity;
            if (initial_bindings_size > 0 && initial_bindings_copy) {
                findall_state.bindings.bindings = malloc(sizeof(binding_t) * initial_bindings_capacity);
                if (findall_state.bindings.bindings) {
                    memcpy(findall_state.bindings.bindings, initial_bindings_copy,
                           sizeof(binding_t) * initial_bindings_size);
                }
            } else {
        ...
        /* Clean up */
        if (initial_bindings_copy) free(initial_bindings_copy);

        /* Build result list: iterate backwards over solutions array to produce forward-ordered list */
        ...
        /* Unify Result with the collected solutions list */
...
% Replaces all occurrences of Pattern (as a list of codes) in Input with Replacement
replace_all_occurrences([], _, _, []).
replace_all_occurrences(Input, Pattern, Replacement, Output) :-
    append(Pattern, Rest, Input),
    !,
    % Found a match - replace it and continue scanning
...
replace_all_occurrences([C|Rest], Pattern, Replacement, [C|Output]) :-
    % No match at this position - keep character and advance
