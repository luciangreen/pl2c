...
- `findall/3`, `assertz/1`, `cut/0` predicates with multiple mathematical operations and combinations of member and multiple rules do not necessarily work.
- There have been no benchmarks performed on pl2c because of lack of cut and other predicates, but despite the good algorithm and light design, further optimisation might minimise the logical if-then C structures to conditions unchecked elsewhere, approaching the speed of C.
...
- `assertz/1` should pass around non-code as variables, but code needs to be recompiled, slowing down the interpreter.
