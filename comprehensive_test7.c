...
bool capital_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    /* Clause 1: capital(A,B) :- country(A,B) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), arg2)) {

            do {
    if (!country_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)}))) { state->failed = true; break; }
            } while (0);
...
bool list_length_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    ...
    /* Clause 2: list_length([A|B],C) :- list_length(B,D),C is D+1 */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)})), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), arg2)) {

            do {
    if (!list_length_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)}))) { state->failed = true; break; }
    if (!is_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("+", 2, (term_t*[]){create_compound("$VAR", 1, (term_t*[]){create_int(3)}), create_int(1)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool reverse_list_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    ...
    /* Clause 2: reverse_list([A|B],C) :- reverse_list(B,D),append(D,[A],C) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)})), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), arg2)) {
            ...
            do {
    if (!reverse_list_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)}))) { state->failed = true; break; }
    if (!append_3(state, create_compound("$VAR", 1, (term_t*[]){create_int(3)}), create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_nil()), create_compound("$VAR", 1, (term_t*[]){create_int(2)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool sum_list_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    ...
    /* Clause 2: sum_list([A|B],C) :- sum_list(B,D),C is A+D */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)})), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), arg2)) {
            ...
            do {
    if (!sum_list_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)}))) { state->failed = true; break; }
    if (!is_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("+", 2, (term_t*[]){create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)})}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool between_3(prolog_state_t* state, term_t* arg1, term_t* arg2, term_t* arg3) {
    /* Clause 1: between(A,B,A) :- A=<B */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), arg2) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg3)) {

            do {
    if (!lte_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
    ...
    /* Clause 2: between(A,B,C) :- A<B,D is A+1,between(D,B,C) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), arg2) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), arg3)) {

            do {
    if (!lt_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)}))) { state->failed = true; break; }
    if (!is_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(3)}), create_compound("+", 2, (term_t*[]){create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_int(1)}))) { state->failed = true; break; }
    if (!between_3(state, create_compound("$VAR", 1, (term_t*[]){create_int(3)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool max_3(prolog_state_t* state, term_t* arg1, term_t* arg2, term_t* arg3) {
    /* Clause 1: max(A,B,A) :- A>=B,! */
    {
        ...
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            ...
            do {
    if (!gte_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)}))) { state->failed = true; break; }
    perform_cut(state);
            } while (0);
            if (!state->failed) return true;
        }
    ...
    /* Clause 2: max(A,B,B) */
    {
        ...
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), arg2) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), arg3)) {
...
bool if_then_else_3(prolog_state_t* state, term_t* arg1, term_t* arg2, term_t* arg3) {
    /* Clause 1: if_then_else(A,B,C) :- A,!,B */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), arg2) &&
            ...
            do {
    if (!$VAR_1(state, create_int(0))) { state->failed = true; break; }
    perform_cut(state);
    if (!$VAR_1(state, create_int(1))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
    ...
    /* Clause 2: if_then_else(A,B,C) :- C */
    {
        ...
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            ...
            do {
    if (!$VAR_1(state, create_int(2))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool append_3(prolog_state_t* state, term_t* arg1, term_t* arg2, term_t* arg3) {
    /* Clause 1: append([],A,A) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_nil(), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg2) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg3)) {
    ...
    /* Clause 2: append([A|B],C,[A|D]) :- append(B,C,D) */
    {
        ...
        if (unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)})), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), arg2) &&
            unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)})), arg3)) {

            do {
    if (!append_3(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool member_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    /* Clause 1: member(A,[A|B]) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)})), arg2)) {
    ...
    /* Clause 2: member(A,[B|C]) :- member(A,C) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)})), arg2)) {

            do {
    if (!member_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool last_elem_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    /* Clause 1: last_elem([A],A) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_nil()), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg2)) {
    ...
    /* Clause 2: last_elem([A|B],C) :- last_elem(B,C) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)})), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), arg2)) {
            ...
            do {
    if (!last_elem_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool nth_3(prolog_state_t* state, term_t* arg1, term_t* arg2, term_t* arg3) {
    ...
    /* Clause 2: nth(A,[B|C],D) :- A>0,E is A-1,nth(E,C,D) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)})), arg2) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(3)}), arg3)) {

            do {
    if (!gt_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_int(0))) { state->failed = true; break; }
    if (!is_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(4)}), create_compound("-", 2, (term_t*[]){create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_int(1)}))) { state->failed = true; break; }
    if (!nth_3(state, create_compound("$VAR", 1, (term_t*[]){create_int(4)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool positive_numbers_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    ...
    /* Clause 2: positive_numbers([A|B],[A|C]) :- A>0,!,positive_numbers(B,C) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)})), arg1) &&
            unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)})), arg2)) {

            do {
    if (!gt_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_int(0))) { state->failed = true; break; }
    perform_cut(state);
    if (!positive_numbers_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
    ...
    /* Clause 3: positive_numbers([A|B],C) :- positive_numbers(B,C) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)})), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), arg2)) {
...
bool map_double_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    ...
    /* Clause 2: map_double([A|B],[C|D]) :- C is A*2,map_double(B,D) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)})), arg1) &&
            unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)})), arg2)) {

            do {
    if (!is_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("*", 2, (term_t*[]){create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_int(2)}))) { state->failed = true; break; }
    if (!map_double_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool all_cities_1(prolog_state_t* state, term_t* arg1) {
    /* Clause 1: all_cities(A) :- findall(B,city(B),A) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1)) {
    ...
    /* findall(B, city(B), A) */
    {
        ...
        /* Enumerate solutions */
        while (true) {
    if (!city_1(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}))) { state->failed = true; break; }
...
bool all_countries_1(prolog_state_t* state, term_t* arg1) {
    /* Clause 1: all_countries(A) :- findall(B,country(B,C),A) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1)) {

            do {
    /* findall(B, country(B,C), A) */
    {
        term_t** solutions = NULL;
        int solution_count = 0;
        prolog_state_t findall_state;
        ...
        /* Enumerate solutions */
        while (true) {
    if (!country_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}))) { state->failed = true; break; }
...
bool find_capitals_1(prolog_state_t* state, term_t* arg1) {
    /* Clause 1: find_capitals(A) :- findall(capital(B,C),capital(B,C),A) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1)) {

            do {
    /* findall(capital(B,C), capital(B,C), A) */
    {
        term_t** solutions = NULL;
        int solution_count = 0;
        prolog_state_t findall_state;
        ...
        /* Enumerate solutions */
        while (true) {
    if (!capital_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}))) { state->failed = true; break; }
...
bool same_country_pair_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    /* Clause 1: same_country_pair(A,B) :- country(C,A),country(C,B),A\=B */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), arg2)) {
            ...
            do {
    if (!country_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("$VAR", 1, (term_t*[]){create_int(0)}))) { state->failed = true; break; }
    if (!country_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)}))) { state->failed = true; break; }
    if (!neq_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool factorial_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    ...
    /* Clause 2: factorial(A,B) :- A>0,C is A-1,factorial(C,D),B is A*D */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), arg2)) {
            ...
            do {
    if (!gt_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_int(0))) { state->failed = true; break; }
    if (!is_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("-", 2, (term_t*[]){create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_int(1)}))) { state->failed = true; break; }
    if (!factorial_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)}))) { state->failed = true; break; }
    if (!is_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("*", 2, (term_t*[]){create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)})}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool fibonacci_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    ...
    /* Clause 3: fibonacci(A,B) :- A>1,C is A-1,D is A-2,fibonacci(C,E),fibonacci(D,F),B is E+F */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), arg2)) {
            ...
            do {
    if (!gt_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_int(1))) { state->failed = true; break; }
    if (!is_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("-", 2, (term_t*[]){create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_int(1)}))) { state->failed = true; break; }
    if (!is_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(3)}), create_compound("-", 2, (term_t*[]){create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_int(2)}))) { state->failed = true; break; }
    if (!fibonacci_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("$VAR", 1, (term_t*[]){create_int(4)}))) { state->failed = true; break; }
    if (!fibonacci_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(3)}), create_compound("$VAR", 1, (term_t*[]){create_int(5)}))) { state->failed = true; break; }
    if (!is_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("+", 2, (term_t*[]){create_compound("$VAR", 1, (term_t*[]){create_int(4)}), create_compound("$VAR", 1, (term_t*[]){create_int(5)})}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool gcd_3(prolog_state_t* state, term_t* arg1, term_t* arg2, term_t* arg3) {
    ...
    /* Clause 2: gcd(A,B,C) :- B>0,D is A mod B,gcd(B,D,C) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), arg2) &&
            ...
            do {
    if (!gt_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_int(0))) { state->failed = true; break; }
    if (!is_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(3)}), create_compound("mod", 2, (term_t*[]){create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)})}))) { state->failed = true; break; }
    if (!gcd_3(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool insert_sorted_3(prolog_state_t* state, term_t* arg1, term_t* arg2, term_t* arg3) {
    /* Clause 1: insert_sorted(A,[],[A]) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_nil(), arg2) &&
            unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_nil()), arg3)) {
    ...
    /* Clause 2: insert_sorted(A,[B|C],[A,B|C]) :- A=<B,! */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)})), arg2) &&
            unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_list(create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}))), arg3)) {
    ...
    /* Clause 3: insert_sorted(A,[B|C],[B|D]) :- A>B,insert_sorted(A,C,D) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), arg1) &&
            unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)})), arg2) &&
            unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)})), arg3)) {

            do {
    if (!gt_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)}))) { state->failed = true; break; }
    if (!insert_sorted_3(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
bool insertion_sort_2(prolog_state_t* state, term_t* arg1, term_t* arg2) {
    ...
    /* Clause 2: insertion_sort([A|B],C) :- insertion_sort(B,D),insert_sorted(A,D,C) */
    {
        int saved_bindings_size = state->bindings.size;
        if (unify(state, create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(1)})), arg1) &&
            unify(state, create_compound("$VAR", 1, (term_t*[]){create_int(2)}), arg2)) {
            ...
            do {
    if (!insertion_sort_2(state, create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)}))) { state->failed = true; break; }
    if (!insert_sorted_3(state, create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_compound("$VAR", 1, (term_t*[]){create_int(3)}), create_compound("$VAR", 1, (term_t*[]){create_int(2)}))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
),capital(uk,A),format(  Capital of UK: ~w
,[A]),write(
Test 2: List Operations
),append([1,2],[3,4],B),format(  append([1,2], [3,4]) = ~w
,[B]),member(2,[1,2,3]),write(  member(2, [1,2,3]) succeeded
),write(
Test 3: Arithmetic
),factorial(5,C),format(  factorial(5) = ~w
,[C]),write(
Test 4: Cut Behavior
),max(10,5,D),format(  max(10, 5) = ~w
,[D]),min(10,5,E),format(  min(10, 5) = ~w
,[E]),write(
Test 5: Recursive Predicates
),reverse_list([1,2,3],F),format(  reverse([1,2,3]) = ~w
,[F]),write(
=== All Tests Completed ===
) */
    {
    ...
    if (!capital_2(state, create_atom("uk"), create_compound("$VAR", 1, (term_t*[]){create_int(0)}))) { state->failed = true; break; }
    if (!format_2(state, create_atom("  Capital of UK: ~w\n"), create_list(create_compound("$VAR", 1, (term_t*[]){create_int(0)}), create_nil()))) { state->failed = true; break; }
    if (!write_1(state, create_atom("\nTest 2: List Operations\n"))) { state->failed = true; break; }
    if (!append_3(state, create_list(create_int(1), create_list(create_int(2), create_nil())), create_list(create_int(3), create_list(create_int(4), create_nil())), create_compound("$VAR", 1, (term_t*[]){create_int(1)}))) { state->failed = true; break; }
    if (!format_2(state, create_atom("  append([1,2], [3,4]) = ~w\n"), create_list(create_compound("$VAR", 1, (term_t*[]){create_int(1)}), create_nil()))) { state->failed = true; break; }
    ...
    if (!write_1(state, create_atom("\nTest 3: Arithmetic\n"))) { state->failed = true; break; }
    if (!factorial_2(state, create_int(5), create_compound("$VAR", 1, (term_t*[]){create_int(2)}))) { state->failed = true; break; }
    if (!format_2(state, create_atom("  factorial(5) = ~w\n"), create_list(create_compound("$VAR", 1, (term_t*[]){create_int(2)}), create_nil()))) { state->failed = true; break; }
    if (!write_1(state, create_atom("\nTest 4: Cut Behavior\n"))) { state->failed = true; break; }
    if (!max_3(state, create_int(10), create_int(5), create_compound("$VAR", 1, (term_t*[]){create_int(3)}))) { state->failed = true; break; }
    if (!format_2(state, create_atom("  max(10, 5) = ~w\n"), create_list(create_compound("$VAR", 1, (term_t*[]){create_int(3)}), create_nil()))) { state->failed = true; break; }
    if (!min_3(state, create_int(10), create_int(5), create_compound("$VAR", 1, (term_t*[]){create_int(4)}))) { state->failed = true; break; }
    if (!format_2(state, create_atom("  min(10, 5) = ~w\n"), create_list(create_compound("$VAR", 1, (term_t*[]){create_int(4)}), create_nil()))) { state->failed = true; break; }
    if (!write_1(state, create_atom("\nTest 5: Recursive Predicates\n"))) { state->failed = true; break; }
    if (!reverse_list_2(state, create_list(create_int(1), create_list(create_int(2), create_list(create_int(3), create_nil()))), create_compound("$VAR", 1, (term_t*[]){create_int(5)}))) { state->failed = true; break; }
    if (!format_2(state, create_atom("  reverse([1,2,3]) = ~w\n"), create_list(create_compound("$VAR", 1, (term_t*[]){create_int(5)}), create_nil()))) { state->failed = true; break; }
    if (!write_1(state, create_atom("\n=== All Tests Completed ===\n"))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
