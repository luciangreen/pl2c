...
bool test_type_checking_0(prolog_state_t* state) {
    ...
    /* If-then-else: atom(hello) -> write(  atom(hello) - passed
) ; write(  atom(hello) - failed
) */
    {
        int saved_size = state->bindings.size;
    if (!atom_1(state, create_atom("hello"))) { state->failed = true; break; }

        if (!state->failed) {
            /* Condition succeeded, execute then branch */
    if (!write_1(state, create_atom("  atom(hello) - passed\n"))) { state->failed = true; break; }
    ...
    /* If-then-else: integer(42) -> write(  integer(42) - passed
) ; write(  integer(42) - failed
) */
    {
        int saved_size = state->bindings.size;
    if (!integer_1(state, create_int(42))) { state->failed = true; break; }

        if (!state->failed) {
            /* Condition succeeded, execute then branch */
    if (!write_1(state, create_atom("  integer(42) - passed\n"))) { state->failed = true; break; }
    ...
    /* If-then-else: number(42) -> write(  number(42) - passed
) ; write(  number(42) - failed
) */
    {
        int saved_size = state->bindings.size;
    if (!number_1(state, create_int(42))) { state->failed = true; break; }

        if (!state->failed) {
            /* Condition succeeded, execute then branch */
    if (!write_1(state, create_atom("  number(42) - passed\n"))) { state->failed = true; break; }
    ...
    /* If-then-else: nonvar(_2744) -> write(  nonvar(5) - passed
) ; write(  nonvar(5) - failed
) */
    {
        int saved_size = state->bindings.size;
    if (!nonvar_1(state, var__2744)) { state->failed = true; break; }

        if (!state->failed) {
            /* Condition succeeded, execute then branch */
    if (!write_1(state, create_atom("  nonvar(5) - passed\n"))) { state->failed = true; break; }
    ...
    /* If-then-else: atomic(hello) -> write(  atomic(hello) - passed
) ; write(  atomic(hello) - failed
) */
    {
        int saved_size = state->bindings.size;
    if (!atomic_1(state, create_atom("hello"))) { state->failed = true; break; }

        if (!state->failed) {
            /* Condition succeeded, execute then branch */
    if (!write_1(state, create_atom("  atomic(hello) - passed\n"))) { state->failed = true; break; }
    ...
    /* If-then-else: is_list([1,2,3]) -> write(  is_list([1,2,3]) - passed
) ; write(  is_list([1,2,3]) - failed
) */
    {
        int saved_size = state->bindings.size;
    if (!is_list_1(state, create_list(create_int(1), create_list(create_int(2), create_list(create_int(3), create_nil()))))) { state->failed = true; break; }

        if (!state->failed) {
            /* Condition succeeded, execute then branch */
    if (!write_1(state, create_atom("  is_list([1,2,3]) - passed\n"))) { state->failed = true; break; }
    ...
    /* If-then-else: ground(foo(1,2)) -> write(  ground(foo(1,2)) - passed
) ; write(  ground(foo(1,2)) - failed
) */
    {
        int saved_size = state->bindings.size;
    if (!ground_1(state, create_compound("foo", 2, (term_t*[]){create_int(1), create_int(2)}))) { state->failed = true; break; }

        if (!state->failed) {
            /* Condition succeeded, execute then branch */
    if (!write_1(state, create_atom("  ground(foo(1,2)) - passed\n"))) { state->failed = true; break; }
...
Testing arithmetic:
),_3464 is abs(-5),format(  abs(-5) = ~w
,[_3464]),_3490 is max(10,5),format(  max(10, 5) = ~w
,[_3490]),_3516 is min(10,5),format(  min(10, 5) = ~w
,[_3516]),_3542 is 2^3,format(  2 ^ 3 = ~w
,[_3542]),_3568 is 10 mod 3,format(  10 mod 3 = ~w
,[_3568]),_3592 is sign(-10),format(  sign(-10) = ~w
,[_3592]),_3616 is round(3),format(  round(3) = ~w
,[_3616]) */
    {
        int saved_bindings_size = state->bindings.size;
    ...
    if (!format_2(state, create_atom("  min(10, 5) = ~w\n"), create_list(var__3516, create_nil()))) { state->failed = true; break; }
    if (!is_2(state, var__3542, create_compound("^", 2, (term_t*[]){create_int(2), create_int(3)}))) { state->failed = true; break; }
    if (!format_2(state, create_atom("  2 ^ 3 = ~w\n"), create_list(var__3542, create_nil()))) { state->failed = true; break; }
    if (!is_2(state, var__3568, create_compound("mod", 2, (term_t*[]){create_int(10), create_int(3)}))) { state->failed = true; break; }
    if (!format_2(state, create_atom("  10 mod 3 = ~w\n"), create_list(var__3568, create_nil()))) { state->failed = true; break; }
    if (!is_2(state, var__3592, create_compound("sign", 1, (term_t*[]){create_int(-10)}))) { state->failed = true; break; }
    if (!format_2(state, create_atom("  sign(-10) = ~w\n"), create_list(var__3592, create_nil()))) { state->failed = true; break; }
    if (!is_2(state, var__3616, create_compound("round", 1, (term_t*[]){create_int(3)}))) { state->failed = true; break; }
    if (!format_2(state, create_atom("  round(3) = ~w\n"), create_list(var__3616, create_nil()))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
...
)),compare(_3794,1,2),format(  compare(~w, 1, 2)
,[_3794]) */
    {
        int saved_bindings_size = state->bindings.size;
        term_t* var__3794 = create_var(state->next_var_id++);
        if (true) {

            do {
    if (!write_1(state, create_atom("\nTesting term comparison:\n"))) { state->failed = true; break; }
    /* If-then-else: 1@<2 -> write(  1 @< 2 - passed
) ; write(  1 @< 2 - failed
) */
    {
        int saved_size = state->bindings.size;
    if (!term_lt_2(state, create_int(1), create_int(2))) { state->failed = true; break; }

        if (!state->failed) {
            /* Condition succeeded, execute then branch */
    if (!write_1(state, create_atom("  1 @< 2 - passed\n"))) { state->failed = true; break; }
    ...
    /* If-then-else: atom@<foo(1) -> write(  atom @< foo(1) - passed
) ; write(  atom @< foo(1) - failed
) */
    {
        int saved_size = state->bindings.size;
    if (!term_lt_2(state, create_atom("atom"), create_compound("foo", 1, (term_t*[]){create_int(1)}))) { state->failed = true; break; }

        if (!state->failed) {
            /* Condition succeeded, execute then branch */
    if (!write_1(state, create_atom("  atom @< foo(1) - passed\n"))) { state->failed = true; break; }
    ...
    if (!compare_3(state, var__3794, create_int(1), create_int(2))) { state->failed = true; break; }
    if (!format_2(state, create_atom("  compare(~w, 1, 2)\n"), create_list(var__3794, create_nil()))) { state->failed = true; break; }
            } while (0);
            if (!state->failed) return true;
        }
        /* Restore bindings for next clause */
